...
function requestFormMarkup(){
 ...
 <fieldset id="request-demon-fields"><legend>데몬리스트 기록</legend><p class="request-guideline-note">등재 신청 전 필독 사항과 기록 가이드라인을 확인해 주세요.</p><div class="request-form-links"><button type="button" class="inline-link" data-page="submission-guide">등재 전 필독 사항</button><button type="button" class="inline-link" data-page="guidelines">기록 규정 및 가이드라인</button></div>
 ...
 <div class="form-field"><label for="request-raw-video">무편집 원본 영상 링크 <span>필수</span></label><input id="request-raw-video" type="url" required placeholder="https://…"><small>상위 150위는 포럼 업로드·Google Drive·YouTube 링크, 그 외는 HTTPS 링크를 사용합니다.</small><a class="inline-link" href="https://gmdkf.rococpy.com/" target="_blank" rel="noopener noreferrer">무편집 원본 영상 업로드 ↗</a></div>
...
function readRequestDraft(){const val=id=>document.getElementById('request-'+id).value.trim();return {nickname:val('nickname'),level:val('level'),percent:+val('percent'),version:val('version'),device:val('device'),fps:+val('fps'),modify:val('modify'),video:val('video'),raw:val('raw-video'),note:val('note')}}
...
function previewForumRequest(){
 ...
 const values=[['닉네임',d.nickname],['레벨',d.level],['퍼센트',d.percent+'%'],['게임 버전',d.version==='2.2'?'2.207':d.version==='2.208'?'2.208+':d.version],['사용한 기기',d.device],['FPS / TPS',d.fps],['CBF / FPS Bypass',{none:'없음',cbf:'CBF 사용','fps-bypass':'FPS Bypass 사용'}[d.modify]],['공개 영상',d.video],['무편집 원본',d.raw],['전달 사항',d.note||'-']];
...
document.addEventListener('input',e=>{if(e.target.id==='request-note')document.getElementById('request-note-count').textContent=e.target.value.length+' / 120자'});
